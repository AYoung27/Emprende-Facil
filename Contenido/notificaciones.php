...
?>
	...
	<?php 
		if (!empty($_SESSION)) {
 			$consulta= sprintf("SELECT count(*) FROM tbl_notificacion where IDUsuario='%s'", $conexion->antiInyeccion($_SESSION['ID']));
 			$num = $conexion->ejecutarconsulta($consulta)->fetch_assoc()['count(*)'];

 			
 			$consulta= sprintf("SELECT Descripcion, Redireccion FROM tbl_notificacion where IDUsuario='%s'", $conexion->antiInyeccion($_SESSION['ID']));
			...
			for ($i=0; $i < $iter; $i++) {
				...
				echo '<a class="dropdown-item" href="'.$data['Redireccion'].'">'.$data['Descripcion'].'</a>';
			}
